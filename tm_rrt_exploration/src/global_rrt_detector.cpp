...
//---------------------------------------------------------------
	ROS_INFO("Run Exploration");
	bool FirstRun = true;
	std::vector<float> frontiers;
	std::vector<float> x_rand, x_nearest, x_new;
	float Xstartx, Xstarty;
	// int i=0;
	float xr,yr;
...
{	
	...
	if(resetSignal == true){		
		...
		points.points.clear();
		V.clear();	
		// clear out the data in the line and the point
		FirstRun = true;
		ros::spinOnce();
		rate.sleep();
	}
	else{	
		if(resetSignal == false && startStopSignal == true ){
			...
			if (FirstRun == true){
				...
				FirstRun = false;	}
//---------------------------------------------------------------
			// do not need to make sure the start stop is working 
			if(FirstRun == false){
				// now after getting the four points then start working on the initial tree growth
				// Main loop
				while (startStopSignal){
					// Sample free
					x_rand.clear();
					...
					if (checking==-1){
						...
						V.push_back(x_new);						
						p.x=x_new[0]; 	   p.y=x_new[1];		  p.z=0.0;
						line.points.push_back(p);
						p.x=x_nearest[0];  p.y=x_nearest[1]; 	p.z=0.0;
						line.points.push_back(p);
					}
					pub.publish(line);  	
					ros::spinOnce();
			...
			if(resetSignal == false && startStopSignal == false)  
					ros::spinOnce();
					rate.sleep();
		}
		ros::spinOnce();
